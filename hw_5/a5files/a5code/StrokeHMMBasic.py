...
class HMM:
    ...
    def label( self, data ):
        ...
        for i in range(len(data)):
            ...
            #use transition probabilities * partial probability from prior day to compute partial probability for today
                ...
                for key in result[i-1]:
                    ...
                    result[i][key] = max_value
                    series_of_states[i][key] = max_key
       

        output = []
        for i in range(len(data)-1,-1,-1):
            if i == len(data)-1:
                transition_state = max_key

            else: 
                transition_state = series_of_states[i+1][max_key]

            output.insert(0,transition_state)
        # print result
        # print len(data)
        # print series_of_states
        # print output
        return output
        ...
        #             #compute max partial probability
        #             max_value = 0
...
class StrokeLabeler:
    ...
    def confusion(self, trueLabels, classifications):
        '''Take in list of truth labels and list of classified labels.
        Return dictionary of how many were correctly classified for each
        feature'''
        result_dict = {}
        result_dict["drawing"] = {}
        result_dict["text"] = {}
        #initialize all values in dictionary to 0
        #Drawing correctly classified as drawing
        result_dict["drawing"]["drawing"] = 0
        #Drawing wrongly classified as text
        result_dict["drawing"]["text"] = 0
        #Text correctly classified
        result_dict["text"]["text"] = 0
        #Text wrongly classified as drawing
        result_dict["text"]["drawing"] = 0
